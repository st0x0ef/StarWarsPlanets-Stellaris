...
public class CustomSpireColumn extends Feature<CustomSpireColumnConfig> {
    ...
    @Override
    public boolean place(FeaturePlaceContext<CustomSpireColumnConfig> context) {
        int i = context.chunkGenerator().getSeaLevel();
        BlockPos blockPos = context.origin();
        WorldGenLevel worldGenLevel = context.level();
        RandomSource randomSource = context.random();
        CustomSpireColumnConfig columnFeatureConfiguration = context.config();
        if (!canPlaceAt(worldGenLevel, i, blockPos.mutable())) {
            ...
            for (BlockPos blockPos2 : BlockPos.randomBetweenClosed(
                ...
                if (m >= 0) {
                    bl2 |= this.placeColumn(worldGenLevel, i, blockPos2, m, columnFeatureConfiguration.reach().sample(randomSource), columnFeatureConfiguration.block());
                }
            }
    ...
    private boolean placeColumn(LevelAccessor level, int seaLevel, BlockPos pos, int distance, int reach, BlockState blockState) {
        boolean bl = false;
            ...
            if (blockPos2 != null) {
                ...
                for (BlockPos.MutableBlockPos mutableBlockPos = blockPos2.mutable(); j >= 0; --j) {
                    if (isAirOrLavaOcean(level, seaLevel, mutableBlockPos)) {
                        ...
                        if (!level.getBlockState(mutableBlockPos).is(BlocksRegistry.INFERNAL_SPIRE.get())) {
                            break;
                        }
